...
You can (and should) exponentiate the results you get from the log-transformation. Doing so will provide the geometric mean and not the artimetic mean.
...
Using `dplyr` it is easy to calculate the median value as well.

```{r}
group_by(mpg, cyl) %>% 
  summarise(median_mpg = median(cty))
```
...
```{r, echo = FALSE}
df2 <- tidyr::gather(df, y, value, -x) %>% filter(y != "y")
...
Use the `mpg` data and make a model to predict `cty` using the variable `hwy` (*do not forget to look at the data before starting making your model*).
